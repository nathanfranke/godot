...
def configure(env):
    ...
    if env["arch"] not in ["arm32", "arm64", "x86_32", "x86_64"]:
        print(
            "Unsupported CPU architecture: "
            + env["arch"]
            + ". Only arm32, arm64, x86_32, and x86_64 are supported on Android."
        )
        from sys import exit

        exit()

    print("Building for Android, platform " + env["ndk_platform"] + " (" + env["arch"] + ")")

    if get_min_sdk_version(env["ndk_platform"]) < 21:
        if env["arch"] == "x86_64" or env["arch"] == "arm64":
            print(
                "WARNING: arch="
                + env["arch"]
    ...
    print("Building for Android, platform " + env["ndk_platform"] + ", architecture " + env["arch"] + ".")
